...
<template>
  <div class="space-y-2">
    ...
    <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-ink/45 p-4" @click.self="open = false">
      <section role="dialog" aria-modal="true" aria-labelledby="generate-plan-heading" class="w-full max-w-md rounded-xl bg-surface p-5 shadow-xl">
        <div class="flex items-start justify-between gap-4">
          <div>
            <h2 id="generate-plan-heading" class="text-xl font-semibold">{{ replaceExisting ? "Replace draft plan" : "Generate next week" }}</h2>
            <p class="mt-1 text-sm text-ink/65">Choose how many meals the AI should plan across the week.</p>
          </div>
          <button type="button" aria-label="Close generation dialog" class="focus-ring rounded-md p-2 hover:bg-field" @click="open = false">
            <X :size="18" aria-hidden="true" />
          </button>
        </div>
        <form class="mt-5 space-y-4" @submit.prevent="generate">
          <label class="block space-y-2">
            <span class="text-sm font-medium">Number of meals</span>
            <input v-model.number="mealCount" type="number" min="1" required class="focus-ring w-full rounded-md border border-ink/15 bg-surface px-3 py-2" />
          </label>
          <p v-if="replaceExisting" class="text-sm text-tomato">The current editable draft and its shopping list will be replaced.</p>
          <p v-if="error" role="alert" class="text-sm text-tomato">{{ error }}</p>
          <div class="flex justify-end gap-2">
            <button type="button" class="focus-ring rounded-md border border-ink/15 px-4 py-2 text-sm font-medium" @click="open = false">Cancel</button>
            <button type="submit" :disabled="busy" class="focus-ring rounded-md bg-moss px-4 py-2 text-sm font-semibold text-white">
